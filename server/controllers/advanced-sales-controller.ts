...
/**
 * Annulla un'operazione di vendita singola e ripristina la cesta/ciclo
 * Permette di spostare la cesta in un FLUPSY diverso se necessario
 */
export async function cancelSaleOperation(req: Request, res: Response) {
  try {
    const { operationId } = req.params;
    const { destinationFlupsyId, destinationPosition } = req.body;

    if (!operationId) {
      return res.status(400).json({
        success: false,
        error: "ID operazione richiesto"
      });
    }

    const opId = parseInt(operationId, 10);
    ...
    const op = saleOperation[0];

    if (op.type !== 'vendita') {
      return res.status(400).json({
        success: false,
        error: "L'operazione non è di tipo vendita"
      });
    }

    // 2. Recupera informazioni sulla cesta
    ...
    .from(baskets)
    .where(eq(baskets.id, op.basketId))
    .limit(1);
    ...
    // 3. Recupera il ciclo collegato
    const cycleData = await db.select()
      .from(cycles)
      .where(eq(cycles.id, op.cycleId))
      .limit(1);

    if (cycleData.length === 0) {
      return res.status(404).json({
        success: false,
        error: "Ciclo non trovato"
      });
    }

    const cycle = cycleData[0];

    // 4. Verifica che il ciclo sia in stato 'closed'
    if (cycle.state !== 'closed') {
      return res.status(400).json({
        success: false,
        error: "Il ciclo non è chiuso - impossibile annullare la vendita"
      });
    }

    // 5. Verifica che l'operazione di vendita sia l'ultima nel ciclo
    const lastOperation = await db.select({
      id: operations.id,
      type: operations.type
    })
    .from(operations)
    .where(eq(operations.cycleId, op.cycleId))
    .orderBy(desc(operations.date), desc(operations.id))
    .limit(1);

    if (lastOperation.length === 0 || lastOperation[0].id !== opId) {
      return res.status(400).json({
        success: false,
        error: "L'operazione di vendita non è l'ultima del ciclo - impossibile annullare"
      });
    }

    // 6. Verifica che non ci sia un DDT inviato collegato
    const processedRef = await db.select({
      id: saleOperationsRef.id,
      advancedSaleId: saleOperationsRef.advancedSaleId
    })
    .from(saleOperationsRef)
    .where(eq(saleOperationsRef.operationId, opId))
    .limit(1);

    if (processedRef.length > 0) {
      // Verifica se c'è un DDT inviato
      const advSale = await db.select()
        .from(advancedSales)
        .where(eq(advancedSales.id, processedRef[0].advancedSaleId))
        .limit(1);

      if (advSale.length > 0 && advSale[0].ddtId) {
        const ddtData = await db.select()
          .from(ddt)
          .where(eq(ddt.id, advSale[0].ddtId))
          .limit(1);

        if (ddtData.length > 0 && ddtData[0].ddtStato === 'inviato') {
          return res.status(400).json({
            success: false,
            error: "Impossibile annullare: DDT già inviato a Fatture in Cloud"
          });
        }
      }
    }

    // 7. Trova l'operazione precedente per recuperare i valori da ripristinare
    const previousOperation = await db.select({
      id: operations.id,
      type: operations.type,
      animalCount: operations.animalCount,
      totalWeight: operations.totalWeight,
    ...
    .from(operations)
    .where(and(
      eq(operations.cycleId, op.cycleId),
      sql`${operations.id} < ${opId}`
    ))
    .orderBy(desc(operations.date), desc(operations.id))
    .limit(1);

    // Valori da ripristinare (usa quelli dell'operazione precedente o quelli della vendita stessa)
    const restoreValues = previousOperation.length > 0 
      ? {
          animalCount: previousOperation[0].animalCount,
          totalWeight: previousOperation[0].totalWeight,
          animalsPerKg: previousOperation[0].animalsPerKg,
          sizeId: previousOperation[0].sizeId
        }
      : {
    ...
    // 8. Determina FLUPSY destinazione (stesso o diverso)
    const targetFlupsyId = destinationFlupsyId || basket.flupsyId;
    const targetPosition = destinationPosition || basket.position;

    // 8.1 Se FLUPSY diverso, verifica che esista
    if (targetFlupsyId !== basket.flupsyId) {
      const destFlupsy = await db.select({
        id: flupsys.id,
      ...
      if (destFlupsy.length === 0) {
        return res.status(400).json({
          success: false,
          error: "FLUPSY di destinazione non trovato"
        });
      }

      if (!destFlupsy[0].active) {
        return res.status(400).json({
          success: false,
          error: "FLUPSY di destinazione non attivo"
        });
      }
    }

    // 8.2 Se posizione specificata, verifica che non sia occupata
    if (destinationPosition && (destinationFlupsyId || destinationPosition !== basket.position)) {
      const occupiedPosition = await db.select({ id: baskets.id, physicalNumber: baskets.physicalNumber })
        .from(baskets)
        .where(and(
      ...
      if (occupiedPosition.length > 0) {
        return res.status(400).json({
          success: false,
          error: `Posizione ${targetPosition} già occupata dalla cesta #${occupiedPosition[0].physicalNumber}`
        });
      }
    }

    // 9. Genera il nuovo cycleCode basato sul FLUPSY destinazione
    const cycleStartDate = cycle.startDate ? new Date(cycle.startDate) : new Date();
    const yearMonth = `${cycleStartDate.getFullYear().toString().slice(-2)}${(cycleStartDate.getMonth() + 1).toString().padStart(2, '0')}`;
    const newCycleCode = `${basket.physicalNumber}-${targetFlupsyId}-${yearMonth}`;

    // 10. Esegui l'annullamento in transazione atomica
    await db.transaction(async (tx) => {
      // 10.1 Elimina entry lotLedger se esiste
      const ledgerKey = `sale_direct_${opId}_${op.lotId}_${op.basketId}`;
      await tx.delete(lotLedger).where(eq(lotLedger.idempotencyKey, ledgerKey));
      console.log(`🗑️ Entry lotLedger eliminata (key: ${ledgerKey})`);

      // 10.2 Elimina riferimento vendita avanzata se esiste
      if (processedRef.length > 0) {
        await tx.delete(saleOperationsRef).where(eq(saleOperationsRef.operationId, opId));
        console.log(`🗑️ Riferimento vendita avanzata eliminato per op ${opId}`);
      }

      // 10.3 Elimina l'operazione di vendita
      await tx.delete(operations).where(eq(operations.id, opId));
      console.log(`🗑️ Operazione vendita ${opId} eliminata`);

      // 10.4 Riapri il ciclo
      await tx.update(cycles)
        ...
        .where(eq(cycles.id, op.cycleId));
      console.log(`🔄 Ciclo ${op.cycleId} riaperto`);
      
      // 10.5 Aggiorna la cesta (potenzialmente in nuovo FLUPSY)
      await tx.update(baskets)
        .set({
          state: 'active',
          currentCycleId: op.cycleId,
          cycleCode: newCycleCode,
          flupsyId: targetFlupsyId,
          position: targetPosition
        })
        .where(eq(baskets.id, op.basketId));
      console.log(`🔄 Cesta ${basket.physicalNumber} ripristinata - FLUPSY: ${targetFlupsyId}, Posizione: ${targetPosition}, CycleCode: ${newCycleCode}`);
    });

    // 11. Recupera i nomi per la risposta
    const destFlupsyName = await db.select({ name: flupsys.name })
      .from(flupsys)
      .where(eq(flupsys.id, targetFlupsyId))
      .limit(1);

    console.log(`✅ Annullamento vendita completato: Operazione ${opId}, Cesta #${basket.physicalNumber}`);

    res.json({
      success: true,
      message: `Vendita annullata con successo. La cesta #${basket.physicalNumber} è stata ripristinata`,
      restoredBasket: {
        id: basket.id,
        ...
        cycleCode: newCycleCode,
        flupsyId: targetFlupsyId,
        flupsyName: destFlupsyName[0]?.name || 'N/A',
        position: targetPosition,
        restoredValues: restoreValues
      }
    });

  } catch (error: any) {
    console.error("Errore nell'annullamento della vendita:", error);
    res.status(500).json({
      success: false,
...
/**
 * Ottiene i FLUPSY disponibili per la destinazione
 */
export async function getAvailableFlupsysForCancellation(req: Request, res: Response) {
  try {
    ...
    console.error("Errore nel recupero FLUPSY:", error);
    res.status(500).json({
      success: false,
      error: error.message || "Errore nel recupero dei FLUPSY"
    });
  }
}