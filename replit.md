...
- **LCI Module (Life Cycle Inventory)** - ECOTAPES Integration: Planned independent module for Life Cycle Inventory management supporting ECOTAPES project requirements. Will provide: materials/equipment inventory with lifecycle tracking, consumables monitoring (energy, fuel, chemicals), automatic production data extraction from existing app (read-only), Excel export in ECOTAPES format, AI-powered insights for sustainability analysis. **Architecture**: Completely isolated module with dedicated `lci_*` database tables, adapter pattern for read-only access to existing data, no modifications to core app. **Documentation**: See `docs/LCI-INTEGRATION-PLAN.md` for detailed implementation plan. **Status**: Planning phase - awaiting approval for development.
