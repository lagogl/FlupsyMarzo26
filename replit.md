...
## Recent Changes
- **2025-11-01**: Finalized Shared Orders Module - Implemented `stato_calcolato` (automatic order status) across all order management interfaces. Status calculates automatically based on deliveries from both apps in `consegne_condivise` table. Backend endpoints (`/api/fatture-in-cloud/orders` and `/api/ordini-condivisi`) aggregate delivery data and compute status ('Aperto', 'Parziale', 'Completato'). Database synchronization completed: populated `quantita_totale` and `cliente_nome` fields in external database for data consistency across all views. Added PostgreSQL trigger to automatically update `stato` field when deliveries are created/modified/deleted, ensuring perfect synchronization between manual and calculated status.
...
- **Shared Orders Module**: Direct database connection to external application for collaborative order management. Features single shared database (Scenario B), automatic residual quantity calculation via SQL view, automatic order status via `stato_calcolato` field (reflects deliveries from both apps), delivery tracking with app origin attribution (`app_origine` field), and UI for managing partial deliveries across multiple applications. Order status updates automatically based on deliveries in `consegne_condivise` table.
