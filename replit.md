...
## Recent Changes
- **2025-11-11**: Implemented Soft Delete for Task Management - Replaced hard delete with soft delete (status='cancelled') for task cancellation to preserve tracciability and enable analytics. Added `cancelled_by` and `cancelled_at` fields to `selection_tasks` table. Backend automatically filters cancelled and completed tasks from operator task list via `/api/operators/:operatorId/tasks` endpoint. External FLUPSY Web app accesses database directly with own SQL queries and has implemented equivalent filtering. Documentation updated (v2.2) with error handling for 404 scenarios when completing cancelled tasks. Fixed Task Completion Bug with PostgreSQL Trigger - Created `check_task_completion()` trigger on `selection_task_assignments` table to automatically update parent task status when all assignments are completed. Prevents UI inconsistencies where tasks with all completed assignments were not showing as completed. Trigger ensures database integrity even when service layer is bypassed, guaranteeing task.status always reflects actual completion state. Trigger name: `assignment_completion_trigger` (AFTER INSERT OR UPDATE on selection_task_assignments).
- **2025-11-10**: Implemented REST API Integration for External App - Completed API-based task management integration replacing direct database access for security and control. External app now accesses tasks exclusively through REST endpoints documented in `docs/EXTERNAL_APP_API_INTEGRATION.md`. Key endpoints include: GET /operators/:operatorId/tasks (retrieve assigned tasks), GET /tasks/:taskId/baskets (view basket details), PATCH /tasks/:taskId/assignments/:assignmentId (start/complete tasks with operatorId validation). Service layer enhanced to track `started_by`/`completed_by` with timestamps, controller enforces mandatory operatorId for in_progress/completed status transitions. Comprehensive documentation provides HTTP examples, error handling patterns, React Native code samples, and best practices for timeout/retry logic. Enhanced Task Tracking Integration - Added `started_by` and `completed_by` fields to `selection_task_assignments` table for complete operator traceability. External app can now track which operator actually started/completed tasks (vs who was assigned). Enables performance analytics: calculate average execution time per operator, identify bottlenecks, and monitor individual productivity. Created optimized indexes for efficient query performance on temporal analysis. Schema fully aligned with external FLUPSY app integration requirements. Dashboard InfoTicker component now displays scrolling task information showing only today's incomplete tasks with full details (baskets, FLUPSY, operators, notes).
