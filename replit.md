...
- **Persistent FLUPSY Preferences (Feb 2026)**: Users can save preferred FLUPSY IDs from the Dashboard. All FLUPSY dropdowns across modules (except Vendite and Vagliatura con Mappa) are filtered to show only preferred FLUPSYs. "Mostra tutti" clears preferences globally. **Schema**: `preferredFlupsyIds` integer array in `user_menu_preferences` table. **API**: `POST /api/menu-preferences/:userId/preferred-flupsys`. **Hook**: `useFlupsyPreferences()` provides `filterFlupsys()` utility. **Files**: `client/src/hooks/use-flupsy-preferences.ts`, `client/src/components/dashboard/FlupsySelector.tsx`.
