...
## Recent Changes
- **2025-11-01**: Finalized Shared Orders Module - Uses `stato_calcolato` for automatic order status based on deliveries from both apps (Delta Futuro and External App). Status updates automatically when any app creates deliveries in `consegne_condivise` table. Fixed quantity and status display to read directly from external database `ordini` table.
