...
- **Basket State Consistency Invariant (Nov 2024)**: All basket state updates MUST write three fields atomically: `state`, `currentCycleId`, `cycleCode`. **Pattern**: When activating a basket, set `state: 'active'`, `currentCycleId: <cycle_id>`, `cycleCode: '<physicalNumber>-<flupsyId>-<YYMM>'`. When freeing a basket, set `state: 'available'`, `currentCycleId: null`, `cycleCode: null`. **Sold basket semantics**: When basket contents are sold, the physical basket becomes `available` (not `active`) because it's freed for reuse. **CycleCode derivation**: Must use actual `basket.flupsyId` from database, never hardcode fallback to `1`. **Enforced in**: routes.ts, selection-controller.ts, screening-controller.ts, database-consistency-service.ts, operations-lifecycle.service.ts. **Database-Level Enforcement**: PostgreSQL trigger `basket_state_triplet_check` prevents partial state updates - raises exception if state='active' without currentCycleId/cycleCode or state='available' with residual references.
- **Centralized Cache Invalidation**: All cache invalidation uses `invalidateAllCaches()` from operations-lifecycle.service.ts. Clears: OperationsCache, BasketsCache, positionCache, unifiedCache, cyclesCache. No scattered cache calls - single point of invalidation.
- **FK Constraints (Data Integrity)**: Full referential integrity via foreign key constraints. Core: cycles→baskets (CASCADE), operations→baskets (CASCADE), operations→cycles (SET NULL). Screening/Selection: source/destination_baskets→baskets (CASCADE), →cycles (SET NULL). Prevents orphan records automatically.
- **Nightly Integrity Check System**: Automated scheduler runs at 03:00 checking: basket state consistency (active without cycle, available with cycle), orphan cycles/operations, cycle state inconsistencies. API: `/api/database/integrity-check` (manual), `/api/database/integrity-check/last` (view results). Service: `nightly-integrity-check.service.ts`.
- **Audit Logging System**: Tracks critical operations in `audit_logs` table. Logs: operation deletions (normal and cascade), basket state changes, cycle creation/closure, emergency deletes. Fields: action, entityType, entityId, oldValues, newValues, metadata, timestamp. API: `/api/database/audit-logs`, `/api/database/audit-logs/:entityType/:entityId`. Service: `audit-log.service.ts`.
- **SQL Migration Documentation**: Formal migration file at `docs/migrations/001_data_integrity_constraints.sql` documents all FK constraints, triggers, and audit table creation with rollback instructions.
