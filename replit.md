...
- **Sales Planning Module (Pianificazione Vendite Ottimizzata)**: Greedy planning engine balancing cash flow, revenue, and order coverage across 6-36 month horizons. Three modes: `cassa` (sell ASAP to meet monthly cash budget), `bilanciato` (orders + cash + hold rest), `ricavo` (hold all, liquidate at end). Uses same growth simulation as growth-projection (per-basket day-by-day SGR + size-specific mortality, hatchery arrivals from `hatchery_arrivals`). Tables: `sales_price_list` (size_code, price_per_kg), `sales_cash_targets` (year, month, min_revenue). Routes: `/api/pianificazione-vendite/*` (plan, price-list CRUD, cash-targets CRUD). Page: `/pianificazione-vendite`. Outputs monthly plan with order shortfall and cash gap indicators. Phase 2 (planned): replace greedy with MILP solver (highs-js) for provably optimal solutions.
