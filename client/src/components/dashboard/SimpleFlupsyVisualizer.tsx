...
export default function SimpleFlupsyVisualizer({ selectedFlupsyIds = [] }: SimpleFlupsyVisualizerProps) {
  ...
  const getActiveSizeForAnimalsPerKg = (animalsPerKg: number | null | undefined): SimpleSize | null => {
    if (animalsPerKg == null || !sizes?.length) return null;
    return sizes.find(size =>
      Number.isFinite(size.minAnimalsPerKg) &&
      Number.isFinite(size.maxAnimalsPerKg) &&
  ...
  // La classificazione usa il limite dell'ultima taglia attiva, mai una soglia locale.
  const hasLargeSize = (basket: any): boolean => {
    ...
    const matched = getActiveSizeForAnimalsPerKg(latestOperation.animalsPerKg);
    const largestAnimalsMax = sizes?.length
      ? Math.min(...sizes.map(size => size.maxAnimalsPerKg).filter(Number.isFinite))
      : null;
  ...
  const getSizeCodeFromAnimalsPerKg = (animalsPerKg: number): string => {
    ...
    // Fuori dai range attivi non assegnare una taglia.
    return 'N/D';
  };

  // Il colore è presentazionale e segue solo l'indice della taglia attiva.
