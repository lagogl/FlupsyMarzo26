    ...
    <div className="container mx-auto py-2 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex-1">
          ...
          <p className="text-xs text-gray-500">Inserimento rapido a foglio elettronico</p>
          <div className="mt-1 inline-flex items-center gap-1.5 px-2 py-0.5 bg-blue-100 border border-blue-300 rounded-full text-xs font-semibold text-blue-700">
            💡 Doppio click su una riga per inserire dati
          </div>
      ...
      {/* Pannello Pivot espandibile con react-pivottable */}
          ...
          <div className="p-4 bg-white overflow-x-auto">
            <PivotTableUI
              data={operationRows.map(row => {
                ...
                const lotName = lot ? `${lot.supplier || ''} ${lot.supplierLotNumber || ''}`.trim() : 'N/A';
                const opDate = new Date(row.date || row.lastOperationDate || '');
                const month = isNaN(opDate.getTime()) ? 'N/A' : 
                  `${opDate.getFullYear()}-${String(opDate.getMonth() + 1).padStart(2, '0')}`;
                
                return {
                  'FLUPSY': row.flupsyName || 'N/A',
                  'Cesta': row.physicalNumber,
                  'Lotto': lotName,
                  'Taglia': row.currentSize || 'N/A',
                  'Mese': month,
                  'Data': row.date || row.lastOperationDate || '',
                  'Tipo Op.': row.type === 'misura' ? 'Misura' : row.type === 'peso' ? 'Peso' : row.type,
                  'Peso (g)': row.totalWeight || 0,
                  'N° Animali': row.animalCount || 0,
                  'Animali/kg': row.animalsPerKg || 0,
                  'Mortalità': row.deadCount || 0,
                  'SGR Peso': row.sgrPeso || 0,
