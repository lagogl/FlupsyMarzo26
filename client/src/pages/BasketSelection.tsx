  ...
  const columns: Column[] = [
    ...
    },
    {
      ...
      cell: (basket) => {
        ...
        // Separa la nota manuale (prima del |) dalla parte automatica (dopo il |)
        const parts = fullNote.split('|');
        const beforePipe = parts[0].trim();
        const afterPipe = parts.slice(1).join('|').trim();

        // Pattern di note auto-generate dal server (non scritte dall'operatore)
        const AUTO_PREFIXES = [
          'Da vagliatura #',
        ...
        const isAutoGenerated = AUTO_PREFIXES.some(p => beforePipe.startsWith(p));

        // Nota manuale: la parte prima del | se non è auto-generata
        const manualNote = isAutoGenerated ? '' : beforePipe;

        // Sezioni del tooltip
        const tooltipAutoNote = isAutoGenerated ? beforePipe : afterPipe ? afterPipe : '';
