  ...
  const columns: Column[] = [
    ...
    },
    {
      ...
      cell: (basket) => {
        ...
        const systemPart = sepIdx !== -1 ? fullNote.substring(0, sepIdx).trim() : '';
        const operatorPart = sepIdx !== -1 ? fullNote.substring(sepIdx + OPERATOR_NOTE_SEPARATOR.length).trim() : '';

        // Pattern di note auto-generate dal server (senza separatore esplicito)
        const AUTO_PREFIXES = [
          'Da vagliatura #',
        ...
        if (sepIdx !== -1) {
          // Nota con separatore: la parte automatica va nel tooltip, quella operatore in cella
          manualNote = operatorPart;
          tooltipAutoNote = systemPart;
        } else {
          // Nessun separatore: classifica in base al prefisso
          const isAutoGenerated = AUTO_PREFIXES.some(p => fullNote.startsWith(p));
          manualNote = isAutoGenerated ? '' : fullNote.trim();
          tooltipAutoNote = isAutoGenerated ? fullNote.trim() : '';
        }
