...
export default function BasketSelection() {
  ...
  const columns: Column[] = [
    ...
    },
    {
      ...
      cell: (basket) => {
        const fullNote = basket.lastOperation?.notes || '';
        if (!fullNote) return <span className="text-muted-foreground">-</span>;

        // Il separatore ufficiale tra nota automatica e nota operatore è |✏️|
        const OPERATOR_NOTE_SEPARATOR = '|✏️|';
        const sepIdx = fullNote.indexOf(OPERATOR_NOTE_SEPARATOR);

        // Pattern di note auto-generate dal server la cui parte "before pipe" NON va in cella
        const PURE_AUTO_PREFIXES = [
          'LOTTO MISTO:',
        ];

        // Pattern di note dove la parte prima del | è leggibile (va in cella) e quella dopo è il dettaglio tecnico
        const AUTO_PREFIXES_WITH_DETAIL = [
          'Da vagliatura #',
          'Chiusura per vagliatura',
          'Vendita da vagliatura',
          'Vendita diretta da vagliatura',
          'Misura da vagliatura',
          'Mescolamento vagliatura',
          'Uscita da vagliatura',
          'Backfill:',
        ];

        let manualNote: string;
        let tooltipAutoNote: string;

        if (sepIdx !== -1) {
          // Nota con separatore ufficiale: sistema→tooltip, operatore→cella
          manualNote = fullNote.substring(sepIdx + OPERATOR_NOTE_SEPARATOR.length).trim();
          tooltipAutoNote = fullNote.substring(0, sepIdx).trim();
        } else {
          // Nessun separatore ufficiale: usa il | semplice come fallback (note vecchio formato)
          const pipeIdx = fullNote.indexOf('|');
          const beforePipe = pipeIdx !== -1 ? fullNote.substring(0, pipeIdx).trim() : fullNote.trim();
          const afterPipe = pipeIdx !== -1 ? fullNote.substring(pipeIdx + 1).trim() : '';

          const isPureAuto = PURE_AUTO_PREFIXES.some(p => fullNote.startsWith(p));
          const hasDetailAfterPipe = AUTO_PREFIXES_WITH_DETAIL.some(p => beforePipe.startsWith(p));

          if (isPureAuto) {
            // Es. nota LOTTO MISTO senza prefisso leggibile → tutto nel tooltip
            manualNote = '';
            tooltipAutoNote = fullNote.trim();
          } else if (hasDetailAfterPipe) {
            // Es. "Da vagliatura #146 | SOPRA VAGLIATURA | LOTTO MISTO: ..."
            // La parte prima del | è già il label (va in cella), il resto nel tooltip
            manualNote = beforePipe;
            tooltipAutoNote = afterPipe;
          } else {
            // Nota senza prefisso noto: trattata come nota operatore (tutto in cella)
            manualNote = fullNote.trim();
            tooltipAutoNote = '';
          }
        }
        ...
        return (
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <span className="text-xs text-gray-700 max-w-[200px] block truncate cursor-help">
                  {manualNote || <span className="text-muted-foreground italic">-</span>}
                </span>
              </TooltipTrigger>
              <TooltipContent side="left" className="max-w-xs p-0 border-0 shadow-lg">
                <div className="bg-white rounded-lg border shadow-xl overflow-hidden text-xs">
                  {manualNote && (
                    <div className="px-3 py-2 bg-blue-50 border-b">
                      <div className="text-blue-600 font-semibold mb-0.5">Nota operatore</div>
                      <div className="text-gray-800">{manualNote}</div>
                    </div>
                  )}
                  {tooltipAutoNote && (
                    <div className="px-3 py-2 bg-gray-50">
                      <div className="text-gray-400 font-semibold mb-0.5">Info automatica</div>
                      <div className="text-gray-600 whitespace-pre-wrap break-words">{tooltipAutoNote}</div>
                    </div>
                  )}
                  {!manualNote && !tooltipAutoNote && (
                    <div className="px-3 py-2 text-gray-400 italic">Nessuna nota</div>
                  )}
                </div>
              </TooltipContent>
            </Tooltip>
