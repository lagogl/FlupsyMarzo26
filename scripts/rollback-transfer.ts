 ...
 * Supporta sia modalità totale che parziale (nuovo ciclo su sorgente).
 *
 * Uso:
...
async function rollbackTransfer(sourceOpId: number) {
  ...
  try {
    ...
    // 1. Leggi operazione sorgente (trasferimento)
    ...
    console.log(`  ID: ${src.id} | Cesta: ${src.basket_id} | Ciclo (vecchio): ${src.cycle_id} | Data: ${src.date}`);

    // 2. Trova tutti i record transfer_in collegati
    const transferInsRes = await client.query(
      `SELECT ll.basket_id, ll.dest_cycle_id, ll.operation_id, ll.idempotency_key
       FROM lot_ledger ll
       WHERE ll.idempotency_key LIKE $1
         AND ll.type = 'transfer_in'`,
      [`transfer_in_${sourceOpId}_%`]
    );
    console.log(`\nRecord transfer_in trovati: ${transferInsRes.rowCount}`);

    const selfKey = `transfer_in_${sourceOpId}_${src.lot_id}_${src.basket_id}_self`;
    let selfRecord: any = null;

    for (const row of transferInsRes.rows) {
      const { basket_id, dest_cycle_id, operation_id, idempotency_key } = row;

      if (idempotency_key === selfKey) {
        // Questo è il record della quota trattenuta dalla sorgente (modalità parziale)
        selfRecord = row;
        console.log(`  ↩ Record self (sorgente) trovato: ciclo nuovo=${dest_cycle_id}, op=${operation_id}`);
        continue;
      }

      // Cesta destinazione normale
      if (operation_id) {
        await client.query(
          `UPDATE operations SET cancelled_at = NOW(), notes = CONCAT(notes, ' | ANNULLATO DA ROLLBACK SCRIPT') WHERE id = $1`,
          [operation_id]
        );
        console.log(`  ✓ Cancellata prima-attivazione ${operation_id} su cesta ${basket_id}`);
      }
      ...
      if (dest_cycle_id) {
        ...
        console.log(`  ✓ Chiuso ciclo destinazione ${dest_cycle_id}`);
      }
    ...
    // 4. Cancella operazione trasferimento sorgente
    await client.query(
      `UPDATE operations SET cancelled_at = NOW(), notes = CONCAT(notes, ' | ANNULLATO DA ROLLBACK SCRIPT') WHERE id = $1`,
      [sourceOpId]
    );
    console.log(`  ✓ Cancellata operazione trasferimento sorgente ${sourceOpId}`);

    // 5. Ripristina ciclo sorgente (il vecchio era sempre chiuso in entrambe le modalità)
    const oldCycleRes = await client.query(
      `SELECT id, state, basket_id FROM cycles WHERE id = $1`,
      [src.cycle_id]
    );

    if (oldCycleRes.rowCount && oldCycleRes.rows[0].state === "closed") {
      await client.query(
      ...
      console.log(`  ✓ Ciclo vecchio sorgente ${src.cycle_id} riaperto, cesta ${src.basket_id} riattivata`);
    }

    // 6. Se modalità parziale: chiudi nuovo ciclo sorgente e cancella prima-attivazione self
    if (selfRecord) {
      const { dest_cycle_id: newSourceCycleId, operation_id: selfOpId } = selfRecord;
      ...
      if (selfOpId) {
        ...
        console.log(`  ✓ Cancellata prima-attivazione self ${selfOpId} (quota sorgente)`);
      }
      ...
      if (newSourceCycleId) {
        ...
        console.log(`  ✓ Chiuso nuovo ciclo sorgente ${newSourceCycleId}`);
      }
      ...
      console.log(`  ✓ Rimosso record lot_ledger transfer_in self`);
    }
